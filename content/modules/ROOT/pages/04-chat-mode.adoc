...
. Select the *hr-vector-db-v1-0* knowledge base from the *Select Document Collections for RAG queries* dropdown.
+
image::click-knowledge-base-drop-down.png[Select knowledge base,link=self,window=blank]
...
====
image::max-token-settings.png[Adjust max tokens,link=self,window=blank]
