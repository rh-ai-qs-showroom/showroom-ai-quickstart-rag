...
. In the sidebar, click the *Chat* menu item.
. Make sure no Document collection is selected: leave the *Select Document Collections for RAG queries* dropdown unset.
...
+
image::chat-direct-result.png[Direct chat answer with no knowledge base and no source citations,role=expand]

== Try it: chat with a knowledge base (RAG)

Follow these steps to chat with RAG retrieval:

. In the sidebar, ensure *Processing mode* is set to *Direct*.
. Select the *hr-vector-db-v1-0* knowledge base from the *Select Document Collections for RAG queries* dropdown. This dropdown only appears once at least one knowledge base exists; see xref:03-knowledge-base.adoc[Knowledge Base] if it's not visible yet.
+
image::click-knowledge-base-drop-down.png[Select knowledge base,role=expand]
...
----
. *Expected result*: the model returns an answer grounded in the retrieved document chunks. A *Searched vector stores* indicator and an expandable *File Search Results* panel appear above the response so you can trace the answer back to the originating document. The response should closely mirror the language in the source documents.
+
image::chat-rag-result.png[RAG answer grounded in the knowledge base showing the searched vector store and file search results,role=expand]
...
image::max-token-settings.png[Adjust max tokens,role=expand]
====
...
|Max Tokens
|Caps the length of the generated response. When RAG retrieval is active, set this as high as possible because retrieved chunks consume a significant portion of the available token budget and a low cap can cause the response to be cut short.
