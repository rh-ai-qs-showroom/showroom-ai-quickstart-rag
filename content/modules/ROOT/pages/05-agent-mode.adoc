...
. In the sidebar, set *Processing mode* to *Agent-based*.
+
image::agent-mode.png[Agent Mode,role=expand]
...
+
image::select-knowledge.png[Select Knowledge Base,role=expand]
...
* Using tools (e.g., web search) without mixing in local document context.
...
"What onboarding steps are specific to the TechSupport department versus HR?"
----
...
. *Without web search*: set *Processing mode* to *Agent-based*, leave web search disabled, and ask:
+
[source,text]
----
...
+
image::websearch-result.png[Agent-based mode calling the web search tool to answer a question beyond its training cutoff,role=expand]
...
[IMPORTANT]
====
Set *Max Tokens* to its maximum value in the sidebar when using Agent-based mode with tools and RAG together.
...
If the agent is still cutting reasoning short, also check *Max Inference Iterations*: it caps how many tool-calling/reasoning loops the agent can run before it must return a final answer.
====

[TIP]
====
Agent-based mode is best suited for multi-part questions that require synthesizing information from several documents or knowledge base sections. If your question is straightforward and single-turn, Direct mode will be faster and simpler.
====
